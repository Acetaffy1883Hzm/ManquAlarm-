...
function stamp(ms,full=false){if(!ms)return '尚未检测';try{return new Intl.DateTimeFormat('zh-CN',{timeZone:S?.zone||undefined,...(full?{month:'2-digit',day:'2-digit'}:{}),hour:'2-digit',minute:'2-digit',...(full?{}:{second:'2-digit'}),hour12:false}).format(new Date(ms));}catch{return new Date(ms).toLocaleTimeString();}}
...
function applyTheme(){const c=S.config,night=c.theme==='dark'||(c.theme==='system'&&matchMedia('(prefers-color-scheme:dark)').matches);document.body.classList.toggle('dark',night);}
function header(){const history=route==='history';$('#header').innerHTML=`<div class="brand"><img src="hazel.png" alt="满区闹钟图标"><div><div class="brand-name">满区闹钟</div><div class="brand-sub">HAZEL LIVE COMPANION</div></div></div><button class="icon-button" data-action="${history?'back':'history'}" aria-label="${history?'返回':'通知记录'}">${icon(history?'close':'history')}</button>`;}
function navigation(){const tabs=[['home','bell','守候'],['schedule','clock','时段'],['sound','sound','声音'],['settings','settings','设置']];$('#nav').innerHTML=tabs.map(([id,i,label])=>`<button class="${route===id?'active':''}" data-route="${id}" ${route===id?'aria-current="page"':''}><span class="icon-wrap">${icon(i)}</span>${label}</button>`).join('');}
function heading(label,title,sub){return `<div class="page-heading"><div class="eyebrow">${label}</div><h1>${title}</h1><p>${sub}</p></div>`;}
function render(){if(!S)return;applyTheme();if(document.body.classList.contains('alarm-page')){renderAlarm();return;}header();navigation();if(route==='home')home();if(route==='schedule')schedule();if(route==='sound')sound();if(route==='settings')settings();if(route==='history')history();}
function statusSummary(){const s=S.snapshot,error=S.networkError,age=Date.now()-(s.checkedAt||0);if(error)return ['待重试','wifi',false];if(!s.checkedAt)return ['尚未检测','clock',false];if(age>Math.max(300000,S.config.pollSeconds*4000))return ['状态待更新','clock',false];if(s.status===1)return ['正在直播','bell',true];if(s.status===2)return ['轮播中 · 不触发开播提醒','play',false];return ['暂未开播','moon',false];}
function alarmAvailable(){const p=S.permissions;return !!S.config.soundWithoutNotifications||!!(p.notifications&&(p.alarmChannelImportance===undefined?p.alarmChannel:p.alarmChannelImportance>0));}
function watchNoticeContent(){
    const status=S.watchNotification?.status||(S.running?'unknown':S.enabled?'interrupted':'stopped');
    const copy={
...
function watchNoticeCard(){return `<section class="card" data-watch-notice>${watchNoticeContent()}</section>`;}
function paintWatchNotice(){const card=$('[data-watch-notice]');if(!card||!S)return;const html=watchNoticeContent();if(card.innerHTML!==html)card.innerHTML=html;}
...
function home(){
    ...
    let html=`<section class="hero"><div class="hero-copy"><div class="eyebrow">A LITTLE CLOSER TO HAZEL</div><h1>${S.enabled?'安心去忙，<br>开播叫你。':'小满一开播，<br>马上叫醒你。'}</h1><div class="tiny-note">${icon('heart')}把期待，交给一声铃响</div></div><img class="hero-art" src="hazel.png" alt="拿着喇叭的灰发小满插画"><div class="hero-chip"><span class="dot ${S.enabled?'green':''}"></span>专属守候 · 灰泽满 Hazel</div></section>
    ...
    <div class="live-info"><span class="dot ${isLive?'live':''}"></span><span class="live-state">${liveLabel}</span><button class="text-button" data-action="refresh">${icon('refresh')}刷新</button></div>${S.snapshot.title?`<p class="live-title">${esc(S.snapshot.title)}</p>`:''}<p class="timestamp" style="margin-top:7px">${S.snapshot.checkedAt?'上次成功检测 '+stamp(S.snapshot.checkedAt)+' · '+esc(S.zone):'尚未连接直播间，点击刷新查看状态'}</p></section>`;
    if(S.ringing)html=`<div class="card warning"><div class="row"><div class="grow"><h3>响铃进行中</h3><p class="sub">点按右侧按钮立即结束</p></div><button class="primary" data-action="dismiss">关闭响铃</button></div></div>`+html;
    ...
    if(S.networkError||S.serviceError)html+=`<div class="card warning"><p>${esc(S.serviceError||S.networkError)}</p></div>`;
    html+=`<div class="quick-actions"><button class="quick" data-action="testDialog">${icon('sound')}<div><strong>响铃测试</strong><small>听一下，放心等</small></div></button><button class="quick" data-action="openLive">${icon('external')}<div><strong>打开直播间</strong><small>去看看小满</small></div></button></div>
...
function schedule(){
    ...
    html+=`<button class="add-rule" data-action="addRule">${icon('plus')}添加提醒时段</button><div class="section-label"><h2>一键添加</h2></div><div class="chips"><button class="chip" data-preset="night">凌晨 01:00–06:00</button><button class="chip" data-preset="overnight">深夜 23:00–07:00</button><button class="chip" data-preset="evening">晚间 19:00–23:00</button></div><section class="card">${setting('提醒时区',`当前：${esc(S.zone)}`,'<button class="text-button" data-action="chooseTimezone">选择时区 ›</button>')}${setting('补报已开播','开启守候或进入时段时，若已经在播也提醒；本场仍只自动提醒一次。',sw('catchUp',c.catchUp,'补报已开播'))}</section><p class="hint">${icon('info')} 关闭补报时，仅提醒在所选时段内开始的直播。起点包含，终点不包含；跨午夜时段按开始那天的星期计算。若接口缺少开播时间，则按首次检测时间判断；首次开启时不猜测已在播的开播时间。</p>`;
    $('#content').innerHTML=html;
}
function sound(){const c=S.config;let html=heading('MAKE IT YOURS','这一声，专门为小满。','选喜欢的铃声，也选一个能听见的音量。');
...
function closeModal(){const m=$('#modal');if(!m)return;m.hidden=true;m.innerHTML='';document.body.style.overflow='';ruleDraft=null;}
...
async function save(patch,notify=false){
    ...
    // Paint the committed result even when an older state request is in flight.
...
function renderZones(query){
 const q=query.trim().toLowerCase();const rows=zoneList.filter(z=>(z.id+' '+(zoneNames[z.id]||'')+' UTC'+z.offset).toLowerCase().includes(q));rows.sort((a,b)=>(zoneNames[a.id]?0:1)-(zoneNames[b.id]?0:1)||a.id.localeCompare(b.id));
...
function privacy(){openModal('隐私与使用说明',`<div class="body-copy"><p><strong>只关注这一位主播。</strong><br>通过 B 站公开接口检测灰泽满 Hazel，UID 1298779265、直播间 1713546334。仅“正在直播”触发开播提醒，轮播不会触发。不需要登录，不读取 Cookie，不访问私信。</p><p><strong>数据留在你的手机。</strong><br>设置、铃声和最多 200 条运行记录保存在应用内，无广告、无统计上报。网络请求仅用于连接 B 站公开直播接口。点击个人空间或直播间时会打开 B 站或浏览器。</p><p><strong>强响铃有明确边界。</strong><br>使用闹钟音量通道、可选振动及全屏通知。应用不会关闭系统勿扰模式，仍需由你在系统中允许闹钟。响铃到期自动停止，同场直播会去重；暂缓提醒是你主动请求的再次提醒。</p><p><strong>通知异常兼容响铃。</strong><br>需你明确开启，通知未获准时仍按所选时段播放闹铃。系统通知权限保持真实状态。悬浮关闭按钮完全可选，仅响铃时显示；可在应用内或系统中随时关闭相关设置。</p><p><strong>无障碍守候辅助。</strong><br>完全可选。只接收本应用自身的窗口变化信号，每约 45 秒检查守候服务；发现中断时请求恢复，失败后限速重试。不读取窗口内容、不执行手势或点击、不监听按键、不截屏。只有你已开启守候时才尝试恢复，不能突破系统强制停止。</p><p><strong>不是云端推送。</strong><br>守候依靠手机后台联网检测。正常情况下延迟约为检测间隔加网络耗时；B 站限制、接口改变、断网、系统休眠或强行停止都可能造成延迟或漏报。关闭“优先保证提醒”可降低耗电，但更容易延迟。</p><p><strong>时间按所选时区计算。</strong><br>跨午夜的时段属于开始那天。缺少接口开播时间时按首次检测时间判断；补报开关允许在进入时段后提醒已经开始的直播。</p><p><strong>文件备份。</strong><br>导出含设置、状态和运行记录，不包含自选铃声本身。恢复时仅恢复设置，不自动开始守候。卸载会删除本地设置与铃声，请先备份。</p><p>图标使用你提供的图片。应用为个人使用制作，与哔哩哔哩及主播无官方关联。</p></div>`);}
async function perform(action){
    if(action==='compatibilityInfo'){compatibilityInfo();return;}
    if(action==='confirmCompatibility'){
        if(compatibilitySaving)return;compatibilitySaving=true;
        const next=compatibilityAction;
        try{await save({soundWithoutNotifications:true});compatibilityAction=null;closeModal();
            if(next==='start')await toggleWatch(true);else if(next)await perform(next);else toast('已开启通知异常兼容响铃');
        }finally{compatibilitySaving=false;}return;
    }
    if(action==='notificationReport'){notificationReport();return;}
    if(action==='reportOnly'||action==='reportWithComponents'){await api('exportNotificationReport',{includeComponents:action==='reportWithComponents'});return;}
    if(action==='notificationHelp'||action==='watchNotificationHelp'){await window.refreshNative(true);notificationHelp();return;}
    ...
    const result=await api(action);if(action==='refresh')toast('正在检查直播间…');if(action==='cancelTest')toast('锁屏测试已取消');if(action==='cancelSnooze')toast('已取消稍后提醒');await window.refreshNative(true);
}
function paintNotificationHelp(){
    const box=$('[data-notification-help]');if(!box||!S)return;
    const p=S.permissions,blocked=p.notificationPolicy==='revoked';
    const advice=$('[data-notification-advice]');if(advice)advice.textContent=blocked?'系统报告通知权限受策略限制，普通授权弹窗无法改变它。当前还不能确定是哪一个系统组件或管理策略设置了限制，请导出排查包继续定位。':'可以从下面的系统设置入口允许通知，再返回重新检查。如果开启后仍恢复关闭，请导出通知排查包。'+(S.xiaomi?' 当前手机优先打开小米通知管理。':'');
    const probe=$('#modal [data-action="notificationProbe"]');if(probe)probe.disabled=!p.notifications;
...
function notificationReport(){
    openModal('导出通知排查包',`<div class="body-copy"><p>用来定位通知开关无法保持的问题。包含满区闹钟的通知状态、最近检查时间、手机系统版本及指定权限组件的版本。</p><p><strong>建议选择“包含权限组件”。</strong><br>会额外复制可读取的系统权限组件安装包，方便核对这台手机使用的具体实现；不读取这些组件的应用数据、聊天、照片或其他个人文件。文件可能有数十 MB。</p><p>选择保存位置后，请把 ZIP 文件发回。不会自动上传，也不会修改系统权限策略。</p></div>`,`<div class="sheet-actions"><button class="secondary" data-action="reportOnly">仅诊断记录</button><button class="primary" data-action="reportWithComponents">包含权限组件</button></div>`);
}
async function toggleWatch(enabled){if(enabled&&!alarmAvailable()){compatibilityInfo('start');return;}if(busy)return;busy=true;try{await api('toggle',{enabled});await window.refreshNative(true);}finally{busy=false;}}
function go(next){route=next;render();window.scrollTo({top:0,behavior:'auto'});}
document.addEventListener('click',async e=>{
    const b=e.target.closest('button');if(!b)return;
    try{
        if(b.dataset.route){go(b.dataset.route);return;}
        if(b.dataset.action){await perform(b.dataset.action);return;}
        if(b.dataset.toggle){const key=b.dataset.toggle;if(key==='enabled')await toggleWatch(!S.enabled);else if(key==='soundWithoutNotifications'&&!S.config[key])compatibilityInfo();else await save({[key]:!S.config[key]});return;}
        if(b.dataset.allDay){await save({allDay:b.dataset.allDay==='true'});return;}
        if(b.dataset.tone){await save({ringtone:b.dataset.tone});return;}
...
document.addEventListener('change',async e=>{try{if(e.target.id==='volume'){await save({volume:Number(e.target.value)});return;}const key=e.target.dataset.setting;if(key){let value=e.target.value;if(['duration','pollSeconds','snoozeMinutes'].includes(key))value=Number(value);await save({[key]:value});}}catch(error){toast(error.message);render();}});
document.addEventListener('input',e=>{if(e.target.id==='tz-search')renderZones(e.target.value);if(e.target.id==='volume')$('#volume-value').innerHTML=`${Number(e.target.value)}<span>%</span>`;});
document.addEventListener('keydown',e=>{if(e.key==='Escape')window.appBack();if(e.key==='Tab'&&$('#modal')&&!$('#modal').hidden){const items=[...$('#modal').querySelectorAll('button,input,select')].filter(x=>!x.disabled),first=items[0],last=items[items.length-1];if(e.shiftKey&&document.activeElement===first){last.focus();e.preventDefault();}else if(!e.shiftKey&&document.activeElement===last){first.focus();e.preventDefault();}}});
window.appBack=()=>{if($('#modal')&&!$('#modal').hidden){closeModal();return;}if(route!=='home'){go('home');return;}api('minimize');};
let refreshPromise=null,refreshQueued=false,forceQueued=false,stateRevision=0;
window.refreshNative=(force=false)=>{
    // Coalesce requests without dropping a forced refresh from a save or resume.
    refreshQueued=true;forceQueued=forceQueued||!!force;
    if(refreshPromise)return refreshPromise;
    refreshPromise=(async()=>{
        do{
            const repaint=forceQueued,revision=stateRevision;
            refreshQueued=false;forceQueued=false;
            try{
                const value=await api('state');
                // A reply captured before a completed save must not restore old settings.
                if(revision!==stateRevision){refreshQueued=true;forceQueued=true;continue;}
                ...
                S=value;configFingerprint=JSON.stringify(S.config);applyTheme();
    ...
    })().then(()=>{refreshPromise=null;if(refreshQueued)return window.refreshNative(forceQueued);},error=>{refreshPromise=null;throw error;});
    return refreshPromise;
};
function renderAlarm(){
    const test=S.alarmTest||S.preview,active=S.ringing||S.preview;
    const key=[test,active,S.alarmTitle,S.config.snoozeMinutes].join('|');
    if(alarmPainted!==key){alarmPainted=key;$('#alarm-root').innerHTML=`<div class="alarm-eyebrow"><span class="dot ${active?'live':''}"></span>${test?'RINGTONE TEST':'HAZEL IS LIVE'}</div><div class="alarm-halo"><img src="hazel.png" alt="小满拿喇叭提醒你"></div><h1>${active?(test?'这一声，听见了吗？':'小满开播啦。'):'响铃已结束。'}</h1><p class="alarm-name">${test?'满区闹钟 · 和正式提醒一样响':'灰泽满 Hazel'}</p><p class="alarm-title">${esc(S.alarmTitle||(test?'确认声音、振动与锁屏显示':'去直播间，和小满见个面。'))}</p><p class="countdown" id="remaining"></p><div class="alarm-actions">${active?(test?`<button class="primary" data-action="dismiss">${icon('check')}听见了，结束测试</button>`:`<button class="primary" data-action="openLive">${icon('external')}去直播间 · 关闭响铃</button><button class="secondary" data-action="snooze">${icon('clock')}${S.config.snoozeMinutes} 分钟后再提醒</button><button class="quiet-close" data-action="dismiss">本场不再响铃</button>`):`<button class="primary" data-action="closeAlarm">知道了</button>`}</div><p class="footnote">${active?'返回键只收起页面，铃声仍继续。<br>请点按上方按钮或通知中的关闭按钮。':'同一场直播不会再次自动响铃。'}</p>`;}
...
window.refreshNative();setInterval(()=>{if(!document.hidden)window.refreshNative();},2000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)window.refreshNative(true);});
