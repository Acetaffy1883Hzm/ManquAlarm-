...
function header() {
  ...
  $("#header").innerHTML = `<div class="brand"><img src="hazel.png" alt="满区闹钟图标"><div><div class="brand-name">满区闹钟</div><div class="brand-sub">HAZEL LIVE COMPANION</div></div></div><button class="icon-button" data-action="${history2 ? "back" : "history"}" aria-label="${history2 ? "返回" : "通知记录"}">${icon(history2 ? "close" : "history")}</button>`;
}
function navigation() {
  const tabs = [["home", "bell", "守候"], ["schedule", "clock", "时段"], ["sound", "sound", "声音"], ["settings", "settings", "设置"]];
...
function statusSummary() {
  const s = S.snapshot, error = S.networkError, age = Date.now() - (s.checkedAt || 0);
  if (error) return ["待重试", "wifi", false];
  if (!s.checkedAt) return ["尚未检测", "clock", false];
  if (age > Math.max(3e5, S.config.pollSeconds * 4e3)) return ["状态待更新", "clock", false];
  if (s.status === 1) return ["正在直播", "bell", true];
  if (s.status === 2) return ["轮播中 · 不触发开播提醒", "play", false];
...
function sound() {
  const c = S.config;
  let html = heading("MAKE IT YOURS", "这一声，专门为小满。", "选喜欢的铃声，也选一个能听见的音量。");
...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (b.dataset.action) {
      await perform(b.dataset.action);
      return;
    }
    if (b.dataset.toggle) {
...
document.addEventListener("change", async (e) => {
  try {
    ...
    if (key) {
      ...
      if (["duration", "pollSeconds", "snoozeMinutes"].includes(key)) value = Number(value);
...
function renderAlarm() {
  ...
  if (alarmPainted !== key) {
    alarmPainted = key;
    $("#alarm-root").innerHTML = `<div class="alarm-eyebrow"><span class="dot ${active ? "live" : ""}"></span>${test ? "RINGTONE TEST" : "HAZEL IS LIVE"}</div><div class="alarm-halo"><img src="hazel.png" alt="小满拿喇叭提醒你"></div><h1>${active ? test ? "这一声，听见了吗？" : "小满开播啦。" : "响铃已结束。"}</h1><p class="alarm-name">${test ? "满区闹钟 · 和正式提醒一样响" : "灰泽满 Hazel"}</p><p class="alarm-title">${esc(S.alarmTitle || (test ? "确认声音、振动与锁屏显示" : "去直播间，和小满见个面。"))}</p><p class="countdown" id="remaining"></p><div class="alarm-actions">${active ? test ? `<button class="primary" data-action="dismiss">${icon("check")}听见了，结束测试</button>` : `<button class="primary" data-action="openLive">${icon("external")}去直播间 · 关闭响铃</button><button class="secondary" data-action="snooze">${icon("clock")}${S.config.snoozeMinutes} 分钟后再提醒</button><button class="quiet-close" data-action="dismiss">本场不再响铃</button>` : `<button class="primary" data-action="closeAlarm">知道了</button>`}</div><p class="footnote">${active ? "返回键只收起页面，铃声仍继续。<br>请点按上方按钮或通知中的关闭按钮。" : "同一场直播不会再次自动响铃。"}</p>`;
  }
